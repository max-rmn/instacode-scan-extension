...
// Shim cross-navigateur : Firefox expose `browser` (WebExtensions, basé Promises)
// et alias `chrome` (avec callbacks) ; Chrome n'expose que `chrome`. On préfère
// `browser` quand il existe, sinon on retombe sur `chrome`. Tous les appels
// ci-dessous sont écrits en style Promise (await/.catch), donc compatibles avec
// les deux namespaces.
...
async function loadConfig() {
  const cfg = await browser.storage.local.get({ panelUrl: DEFAULT_URL, token: '' });
...
async function restore() {
  ...
  if (error || !tab) return;
  try {
    const resp = await browser.tabs.sendMessage(tab.id, { type: 'status' });
    if (!resp) return;
    state.tabId = tab.id;
    if (resp.running) {
      setScanning(true);
